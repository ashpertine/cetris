...
static ttshape SKEW_LAYOUT[BLOCK_COUNT] = {{0, 0}, {0, 1}, {1, 1}, {1, 2}}; // also known as Z
static ttshape INVERSESKEW_LAYOUT[BLOCK_COUNT] = {{1, 0}, {1, 1}, {0, 1}, {0, 2}}; // also known as S
...
Tetromino get_rand_shape(Tetromino *prev_shape) {
  srand(time(NULL));
  Tetromino new = (Tetromino)(rand() % (NUM_TETROMINOS + 1));
  if(prev_shape == NULL) return new;
...
void wprint_lower_tetrimino(WINDOW *local_win, ttshape_state *shape_state, int fact) {
  int curr_y = shape_state->y;
  if(curr_y + fact >= EFFECTIVE_HEIGHT) return;
...
void init_main_win_act(WINDOW *main_win, ttshape_state *state) {
  ...
  while (running) {
    ...
    tick_counter++;
    if(tick_counter >= (unsigned) fall_interval && state->y + y_fact < EFFECTIVE_HEIGHT) {
      wprint_lower_tetrimino(main_win, state, y_fact);
      tick_counter = 0;
    }
...
int main() {
  init_settings();
  ttshape_state state = {
    .current_shape = get_rand_shape(NULL),
    .x = 1,
    .y = 1,
    .longest_x = 1
  };
